...
def preprocess_frame(image, target_height=224, target_width=224):
    ...
    image -= np.array([0.456, 0.406, 0.485])
    image /= np.array([0.225, 0.229, 0.224])
...
def extract_full_feature(encoder):
    ...
    for i, video in enumerate(videos):
        ...
        # 视频特征的shape是max_frames x 2048
        # 如果帧的数量小于max_frames，则剩余的部分用0补足
        feats = np.zeros((max_frames, frame_size), dtype='float32')
